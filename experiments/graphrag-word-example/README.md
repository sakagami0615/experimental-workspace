...
`data/sample.json` にノード情報を定義する。エッジは DB によって扱いが異なる（後述）。
...
| エッジ定義 | **LLM が自動抽出** | JSON に手動定義 | JSON に手動定義 |
...
- **エッジ**: JSON の `edges` に手動で定義し、`RELATE` 文で登録する。
...
- **エッジ**: JSON の `edges` に手動で定義し、EdgeQL の `UPDATE ... SET { related_concepts += ... }` でリンクを設定する。
